(() => {
  ...
  const defaultState = () => ({ version:3, updatedAt:null, settings:{ gaikaexEntered:false }, gaikaex:{}, clickfx:{}, oneaccount:{} });
  let state = loadState();
  let saveTimer;
  ...
  function loadState() {
    try {
      ...
      return {
        ...defaultState(), ...parsed, version:3,
        settings:{ ...defaultState().settings, ...(parsed.settings || {}) },
        gaikaex:{ ...(parsed.gaikaex || {}) },
        clickfx:{ ...(parsed.clickfx || {}) },
        oneaccount:{ ...(parsed.oneaccount || {}) }
      };
  ...
  // GMO外貨 v3: old combined cost is shown as FX cost until the user edits the new split fields.
  function gaikaFxCost(r={}) { return hasValue(r.fxCost) ? num(r.fxCost) : num(r.cost); }
  function gaikaCfdCost(r={}) { return num(r.cfdCost); }
  function calcGaika(r={}) {
    const fxCost=gaikaFxCost(r), cfdCost=gaikaCfdCost(r), reward=num(r.reward);
    const cost=fxCost+cfdCost;
    return {fxCost,cfdCost,cost,reward,net:reward-cost};
  }
  ...
  function onePnl(r={}) {
    if (hasValue(r.investmentPnl)) return num(r.investmentPnl);
    if (hasValue(r.cost)) return -num(r.cost);
    return 0;
  }
  function calcOne(r={}) {
    const entered=!!r.entered, pnl=onePnl(r), pts=num(r.points);
    return {entered,pnl,points:pts,net:pnl+pts};
  }
  ...
  function updateKpis() {
    ...
    net.textContent=yen(t.net);
    net.classList.toggle('positive',t.net>0);
  ...
  function renderGaikaex() {
    let recorded=0,cost=0,reward=0;
    Object.values(state.gaikaex).forEach(r=>{
      const c=calcGaika(r);
      if (hasValue(r.fxCost)||hasValue(r.cfdCost)||hasValue(r.cost)||hasValue(r.reward)||hasValue(r.note)) recorded++;
      cost+=c.cost; reward+=c.reward;
    });
    const summary=`<div class="summary-strip"><div class="summary-item"><span>記録日数</span><strong>${recorded}日</strong></div><div class="summary-item"><span>総コスト</span><strong>${yen(cost)}</strong></div><div class="summary-item"><span>総当選額</span><strong>${yen(reward)}</strong></div><div class="summary-item"><span>ネット</span><strong class="${reward-cost>=0?'positive':'negative'}">${yen(reward-cost)}</strong></div></div>`;
    const rows=businessDays(CAMPAIGNS.gaikaex.start,CAMPAIGNS.gaikaex.end).map(date=>{
      const r=state.gaikaex[date]||{}, c=calcGaika(r);
      const fxDisplay=hasValue(r.fxCost)?r.fxCost:(hasValue(r.cost)?r.cost:'');
      return `<tr class="${isToday(date)?'today-row':''}"><td class="date-cell"><strong>${fmtDate(date)}</strong><span>${date}</span></td><td><input class="money-input signed-number" type="number" step="1" data-bucket="gaikaex" data-key="${date}" data-field="fxCost" value="${escapeAttr(fxDisplay)}" placeholder="0"></td><td><input class="money-input signed-number" type="number" step="1" data-bucket="gaikaex" data-key="${date}" data-field="cfdCost" value="${escapeAttr(r.cfdCost)}" placeholder="0"></td><td><input class="money-input" type="number" min="0" step="1" data-bucket="gaikaex" data-key="${date}" data-field="reward" value="${escapeAttr(r.reward)}" placeholder="0 / 999"></td><td class="inline-result"><strong class="${c.net>=0?'positive':'negative'}">${yen(c.net)}</strong></td><td><input type="text" data-bucket="gaikaex" data-key="${date}" data-field="note" value="${escapeAttr(r.note)}" placeholder="メモ"></td></tr>`;
    }).join('');
    document.querySelector('#view-gaikaex').innerHTML=campaignHeader('gaikaex','毎日500名に999円','FXコスト / CFDコストを日次記録')+summary+`<div class="panel"><div class="panel-head"><div><h3>キャンペーン設定</h3><p>期間中のエントリーは最初の1回だけ</p></div></div><div class="setting-row"><div class="setting-copy"><strong>エントリー済み</strong></div><label class="toggle"><input type="checkbox" data-setting="gaikaexEntered" ${state.settings.gaikaexEntered?'checked':''}><span class="slider"></span></label></div></div><div class="panel"><div class="panel-head"><div><h3>営業日ごとの実績</h3><p>FXとCFDの実コスト、当選額だけを簡潔に記録します。コストは負数も入力できます。</p></div></div><div class="table-wrap gaika-table"><table><thead><tr><th>営業日</th><th>FXコスト ¥</th><th>CFDコスト ¥</th><th>当選 ¥</th><th>ネット</th><th>メモ</th></tr></thead><tbody>${rows}</tbody></table></div></div>`;
  }
  ...
  function renderOneAccount() {
    ...
    const rows=ONE_WEEKS.map((w,i)=>{
      ...
      const pnlValue=hasValue(r.investmentPnl)?r.investmentPnl:(hasValue(r.cost)?-num(r.cost):'');
    ...
    document.querySelectorAll('[data-bucket]').forEach(el=>{
      ...
      if(el.type==='checkbox') {
        el.addEventListener('change',()=>{
          rec(el.dataset.bucket,el.dataset.key)[el.dataset.field]=el.checked;
          saveState(false);
          refreshAfterEdit(el.dataset.bucket);
        });
      } else {
        el.addEventListener('input',()=>{
          const r=rec(el.dataset.bucket,el.dataset.key);
          r[el.dataset.field]=el.value;
          // Once split GMO外貨 cost is edited, stop using legacy combined cost.
          if(el.dataset.bucket==='gaikaex' && el.dataset.field==='fxCost' && hasValue(r.cost)) delete r.cost;
          queueSave();
        });
  ...
  function importData(file) {
    ...
    reader.onload=()=>{
      try {
        ...
        state={...defaultState(),...parsed,version:3,settings:{...defaultState().settings,...(parsed.settings||{})},gaikaex:{...(parsed.gaikaex||{})},clickfx:{...(parsed.clickfx||{})},oneaccount:{...(parsed.oneaccount||{})}};
        saveState(false); renderAll(); toast('バックアップを復元しました');
      } catch(_) { alert('JSONを読み込めませんでした。'); }
    };
  ...
  document.querySelector('#resetBtn').addEventListener('click',()=>{
    if(!confirm('このブラウザに保存したキャンペーン記録をすべて削除します。続けますか？'))return;
